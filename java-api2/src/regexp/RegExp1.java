...
public class RegExp1 {
	public static void main(String[] args) {
		...
		String[] pattern = {
				...
				"c[0-9]", // c와 숫자로 구성된 두자리 문자열(위와동일)
				"c.*t", // c로 시작하고 t로 끝나는 모든 문자열
				"[b|c].*", //b 또는 c로 시작하는 문자열
				"[bc].*" , // (위와 동일)
				"[b-c].*" , // (위와 동일)
				".*a.*" , // a를 포함하는 모든 문자열 * : 0 또는 그 이상의 문자 null 포함
				".*a.+", // a를 포함하는 모든 무자열  + : 1 또는 그이상의 문자 null 허용하지 않음.
						 // a 뒤에 반드시 한개 이상의 문자가 와야 하므로 a로 끝나는 단어는 제외
				"[b|c].{2}", // b 또는 c로 시작하는 세자리 문자열
				"[^b|c].*" // b 또는 c로 시작하지 않는 문자열
		};
		for (int i = 0; i < pattern.length; i++) {
			Pattern p = Pattern.compile(pattern[i]);  // Shallow copy 얇은 카피
			System.out.println();
